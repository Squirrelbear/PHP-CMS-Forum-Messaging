...
if($_SESSION["status"] == 1)
{
	...
	if($_SESSION["dtp"] != "p" || $_SESSION["dtp"] != "t" || $_SESSION["dtp"] != "d")
	{
		// do nothing
	}else {
	...
	$discussion = $_SESSION["d"];
	$topic = $_SESSION["t"];
	
	//print_r($_SESSION);
	//echo "Debug Discussion ID - Session: " . $_SESSION["disucssion;
	echo "<br>Debug Discussion ID: " . $_SESSION["d"] . "            " . $discussion;
	echo "<br>Debug Topic ID: " . $_SESSION["t"] . "            " . $topic;
	...
	{		
		...
		// go up a level
		$add = "<table><tr><td><form method='post' action='up.php'>";
		...
		$add .= "<input type='hidden' name='d' value='" . $_SESSION["d"] . "'>";
		$add .= "<input type='hidden' name='w' value='p'>";
		...
		if(count($result) > 0)
		{
			...
			foreach($result as $field)
			{
				...
				$tbl .= $field[name] . "</td>";
					...
					$tbl .= "<td>";
					$tbl .= "<form method='post' action='view_topic.php'>";
		...
		if(count($result) > 0)
		{
			...
			foreach($result as $field)
			{
				...
				// create view item button
				$tbl .= "<td>";
				$tbl .= "<form method='post' action='view_discussion.php'>";
